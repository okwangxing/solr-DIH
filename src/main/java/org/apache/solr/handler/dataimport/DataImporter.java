...
public class DataImporter {
	...
	DataSource getDataSourceInstance(Entity key, String name, Context ctx) {
		...
		if (p == null)
			throw new DataImportHandlerException(SEVERE, "No dataSource :" + name + " available for entity :" + key.getName());
	...
	private void checkWritablePersistFile(SolrWriter writer, DIHProperties dihPropWriter) {
		if (isDeltaImportSupported && !dihPropWriter.isWritable()) {
			throw new DataImportHandlerException(SEVERE, "Properties is not writable. Delta imports are supported by data config but will not work.");
		}
	}
