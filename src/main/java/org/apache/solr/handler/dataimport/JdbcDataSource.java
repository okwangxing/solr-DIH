...
public class JdbcDataSource extends DataSource<Iterator<Map<String, Object>>> {
	...
	protected Callable<Connection> createConnectionFactory(final Context context, final Properties initProps) {
		...
		if (url == null && jndiName == null)
			throw new DataImportHandlerException(SEVERE, "JDBC URL or JNDI name has to be specified");
		...
		return factory = new Callable<Connection>() {
			...
			public Connection call() throws Exception {
				...
				try {
					...
					// DriverManager does not allow you to use a driver which is
					// not loaded through
					// the class loader of the class which is trying to make the
					// connection.
					// This is a workaround for cases where the user puts the
					// driver jar in the
