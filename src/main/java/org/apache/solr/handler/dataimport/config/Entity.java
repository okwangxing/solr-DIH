...
public class Entity {
	...
	public Entity(boolean docRootFound, Element element, DataImporter di, Entity parent) {
		this(docRootFound, element, di, parent, false, null, null);
	}

	public Entity(boolean docRootFound, Element element, DataImporter di, Entity parent, boolean sharedEntity, String dataSourceName, Map<String, String> modAttributes) {
		...
		if (!sharedEntity) {
			...
			this.name = dataSourceName + "_" + modName;
		}
		...
		if (modAttributes == null) {
			modAttributes = ConfigParseUtil.getAllAttributes(element);
		}
		// datasource 未命名,就设置null
		...
		for (Element elem : n) {
			...
			if (di.getSchema() != null) {
				if (fieldBuilder.getNameOrColumn() != null && fieldBuilder.getNameOrColumn().contains("${")) {
					...
					SchemaField schemaField = di.getSchemaField(fieldBuilder.getNameOrColumn());
					if (schemaField != null) {
		...
		if (di.getSchema() != null) {
			...
			if (uniqueKey != null) {
				...
				// if no fields are mentioned . solr uniqueKey is same as dih
				// 'pk'
				for (EntityField field : fields) {
					if (field.getName().equals(modPkMappingFromSchema)) {
						modPkMappingFromSchema = field.getColumn();
						// get the corresponding column mapping for the solr
						// uniqueKey
						// But if there are multiple columns mapping to the solr
						// uniqueKey,
						// it will fail
		...
		for (Element elem : n) {
			modEntities.add(new Entity((docRootFound || this.docRoot), elem, di, this));
		}
