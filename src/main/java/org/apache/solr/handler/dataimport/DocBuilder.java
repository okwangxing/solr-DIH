...
public class DocBuilder {
	...
	public void execute() {
		try {
			...
			// we must not do a delete of *:* multiple times if there are
			// multiple root entities to be run
			...
			for (Entity e : config.getEntities()) {
				EntityProcessorWrapper epw = getEntityProcessorWrapper(e);
				...
				closeEntityProcessorWrappers(epw);
			}
	...
	private void closeEntityProcessorWrappers(EntityProcessorWrapper epw) {
		epw.close();
	...
	private void finish(Map<String, Object> lastIndexTimeProps) {
		...
		statusMessages.put("", "Indexing completed. Added/Updated: " + importStatistics.docCount + " documents. Deleted " + importStatistics.deletedDocCount + " documents.");
		...
		try {
			...
			statusMessages.put("error", "Could not write property file. Delta imports will not work. " + "Make sure your conf directory is writable");
		}
	}
	...
	@SuppressWarnings("unchecked")
	private void buildDocument(VariableResolver vr, DocWrapper doc, Map<String, Object> pk, EntityProcessorWrapper epw, boolean isRoot, ContextImpl parentCtx,
			List<EntityProcessorWrapper> entitiesToDestroy) {
	...
	private void addFields(Entity entity, DocWrapper doc, Map<String, Object> arow, VariableResolver vr) {
		for (Map.Entry<String, Object> entry : arow.entrySet()) {
			...
			if (field == null && dataImporter.getSchema() != null) {
				// This can be a dynamic field or a field which does not have an
				// entry in data-config ( an implicit field)
	...
	private String findMatchingPkColumn(String pk, Map<String, Object> row) {
		...
		for (String columnName : row.keySet()) {
			if (columnName.endsWith("." + pk) || pk.endsWith("." + columnName)) {
				if (resolvedPk != null)
					throw new IllegalArgumentException(String.format(Locale.ROOT, "deltaQuery has more than one column (%s and %s) that might resolve to declared primary key pk='%s'", resolvedPk,
							columnName, pk));
	 ...
	 * </p>
	 * <p>
	 * Note: In our definition, unique key of Solr document is the primary key
	 * of the top level entity (unless skipped using docRoot=false) in the Solr
	...
	public Set<Map<String, Object>> collectDelta(EntityProcessorWrapper epw, VariableResolver resolver, Set<Map<String, Object>> deletedRows) {
		...
		// all that we have captured is useless (in a sub-entity) if no rows in
		// the parent is modified because of these
		...
		// Do not use entity.isDocRoot here because one of descendant entities
		// may set rootEntity="true"
